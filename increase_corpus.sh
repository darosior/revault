...
for i in seq 10; do
    cargo test test_transaction_chain;
done;

cargo +nightly fuzz run parse_cancel -- -runs=$RUNS -max_len=$MAX_LEN;
cargo +nightly fuzz run parse_emergency -- -runs=$RUNS -max_len=$MAX_LEN;
cargo +nightly fuzz run parse_unvault_emergency -- -runs=$RUNS -max_len=$MAX_LEN;
cargo +nightly fuzz run parse_spend -- -runs=$RUNS -max_len=$MAX_LEN;
